...
HRESULT space::init()
{
	_background = IMAGEMANAGER->findImage("바탕");
...
void space::render() 
{
	...
	//덜덜 거리는 현상이 발생할 수 있기 때문에 나머지 연산으로 더 정밀하게 잡아준다
	...
	switch (direction)
	{
		...
		case DIRECTION_LEFT_MIDDLE:
			rcSour[0] = RectMake(_mapSizeWidth + x, y, _mapSizeWidth, _mapSizeHeight);
